...
if (!['lax', 'strict', 'none'].includes(COOKIE_SAME_SITE)) throw new Error('COOKIE_SAME_SITE must be lax, strict, or none.')
if (COOKIE_SAME_SITE === 'none' && !IS_PRODUCTION) throw new Error('COOKIE_SAME_SITE=none requires production HTTPS.')
...
const setSessionCookie = (res, token) => res.cookie('ledgerly_session', token, { httpOnly: true, secure: IS_PRODUCTION, sameSite: COOKIE_SAME_SITE, maxAge: 7 * 24 * 60 * 60 * 1000, path: '/' })
const setCsrfCookie = (res, token = randomBytes(32).toString('hex')) => { res.cookie('ledgerly_csrf', token, { httpOnly: false, secure: IS_PRODUCTION, sameSite: COOKIE_SAME_SITE, maxAge: 7 * 24 * 60 * 60 * 1000, path: '/' }); return token }
...
const clearSession = (res) => res.clearCookie('ledgerly_session', { httpOnly: true, secure: IS_PRODUCTION, sameSite: COOKIE_SAME_SITE, path: '/' })
...
const adminAuth = async (req, res, next) => {
  const email = (await db.prepare('SELECT email FROM users WHERE id = ?').get(req.userId))?.email || ''
...
app.get('/api/me', auth, async (req, res) => {
  const user = await db.prepare('SELECT id, name, email, pin_hash FROM users WHERE id = ?').get(req.userId)
  if (!user) return res.status(401).json({ error: 'Authentication required.' })
  res.json({ id: user.id, name: user.name, email: user.email, needsPin: !user.pin_hash, requiresPin: Boolean(user.pin_hash) })
})
app.get('/api/admin/me', auth, adminAuth, async (req, res) => {
  const user = await db.prepare('SELECT id, name, email FROM users WHERE id = ?').get(req.userId)
  res.json({ admin: true, user: { id: user.id, name: user.name, email: user.email } })
})

app.post('/api/consent', writeRateLimit, async (req, res) => {
  ...
  await db.prepare('INSERT INTO consent_records (policy_version, analytics, marketing, preferences) VALUES (?, ?, ?, ?)').run(policyVersion, ...values)
...
app.post('/api/auth/signup', authRateLimit, async (req, res) => {
  ...
  try {
    ...
    const result = await db.prepare('INSERT INTO users (name, email, password_hash, pin_hash) VALUES (?, ?, ?, ?)').run(name, email, pinHash, pinHash)
    setSessionCookie(res, issueToken(result.lastInsertRowid))
    res.status(201).json({ name, email, needsPin: false })
  } catch (error) {
    if (error.code === 'SQLITE_CONSTRAINT_UNIQUE' || error.code === '23505' || error.message?.includes('UNIQUE constraint failed')) return res.status(409).json({ error: 'An account with that email already exists.' })
...
app.post('/api/auth/login-email', authRateLimit, async (req, res) => {
  const email = text(req.body.email, 254).toLowerCase()
  const user = await db.prepare('SELECT name, email, pin_hash FROM users WHERE email = ?').get(email)
...
app.post('/api/auth/login-pin', authRateLimit, async (req, res) => {
  const email = text(req.body.email, 254).toLowerCase()
  const pin = typeof req.body.pin === 'string' ? req.body.pin : ''
  const user = await db.prepare('SELECT id, name, email, pin_hash FROM users WHERE email = ?').get(email)
...
app.post('/api/auth/set-pin', authRateLimit, auth, async (req, res) => {
  ...
  const user = await db.prepare('SELECT pin_hash FROM users WHERE id = ?').get(req.userId)
  if (user?.pin_hash) return res.status(409).json({ error: 'A PIN is already set for this account.' })
  if (await isLeakedPassword(pin)) return res.status(400).json({ error: 'This PIN has appeared in a known data breach. Please choose a different one.' })
  const pinHash = await bcrypt.hash(pin, 12)
  await db.prepare('UPDATE users SET pin_hash = ? WHERE id = ?').run(pinHash, req.userId)
  res.json({ ok: true })
})
...
app.post('/api/transactions', writeRateLimit, auth, async (req, res) => {
  ...
  const result = await db.prepare('INSERT INTO transactions (user_id, date, income, expenses, savings, note) VALUES (?, ?, ?, ?, ?, ?)').run(req.userId, date, income, expenses, savings, note)
...
app.delete('/api/transactions/:id', auth, async (req, res) => {
  ...
  const entry = await db.prepare('SELECT income, expenses, savings FROM transactions WHERE id = ? AND user_id = ?').get(req.params.id, req.userId)
  if (!entry) return res.status(404).json({ error: 'Entry not found.' })
  if (kind === 'all') await db.prepare('DELETE FROM transactions WHERE id = ? AND user_id = ?').run(req.params.id, req.userId)
  else {
    const field = kind === 'income' ? 'income' : 'expenses'
    if (kind === 'income') await db.prepare('UPDATE transactions SET income = 0, savings = 0 WHERE id = ? AND user_id = ?').run(req.params.id, req.userId)
    else await db.prepare(`UPDATE transactions SET ${field} = 0 WHERE id = ? AND user_id = ?`).run(req.params.id, req.userId)
    if (kind === 'income' ? entry.expenses === 0 : entry.income === 0 && entry.savings === 0) await db.prepare('DELETE FROM transactions WHERE id = ? AND user_id = ?').run(req.params.id, req.userId)
  }
  res.json({ ok: true })
})
...
app.get('/api/summary', auth, async (req, res) => {
  ...
  const iso = dubaiIso
  const sum = async (from, expression) => (await db.prepare(`SELECT COALESCE(SUM(${expression}), 0) AS total FROM transactions WHERE user_id = ? AND date >= ? AND date <= ?`).get(req.userId, iso(from), iso(today))).total
  res.json({ today: await sum(today, 'expenses'), todayIncome: await sum(today, 'income - savings'), todaySavings: await sum(today, 'savings'), week: await sum(startOfWeek, 'expenses'), month: await sum(startOfMonth, 'expenses'), monthIncome: await sum(startOfMonth, 'income - savings'), monthSavings: await sum(startOfMonth, 'savings') })
})

app.get('/api/calendar', auth, async (req, res) => {
  const month = /^\d{4}-\d{2}$/.test(String(req.query.month || '')) ? String(req.query.month) : dubaiIso().slice(0, 7)
  const rows = await db.prepare('SELECT id, date, income, expenses, savings, note FROM transactions WHERE user_id = ? AND date LIKE ? ORDER BY date ASC, id ASC').all(req.userId, `${month}-%`)
...
if (!process.env.VERCEL) app.listen(PORT, () => log(`Ledgerly API listening on http://localhost:${PORT}`))
